...
contract TokenTest is MemeFactoryTest {
    ...
    function test_FindBestPath_IncludesVaporDEXAdapter() public {
        ...
        uint256 maxSteps = 1;
        DexAggregator.FormattedOffer memory offer = dexAggregator.findBestPath(
            amountIn,
            tokenIn,
            tokenOut,
            maxSteps
        ...
        assertTrue(
            offer.amounts[offer.amounts.length - 1] > 0,
            "No amounts found"
        );
    ...
    function test_SwapNoSplitFromAVAX_StratMember_TradingStarted() public {
        ...
        assertTrue(
            token.balanceOf(address(_jose)) == trade.amountOut,
            "AmountOut not received"
        );
        ...
        offer = dexAggregator.findBestPath(
            amountIn,
            tokenIn,
            tokenOut,
            maxSteps
        ...
        assertTrue(token.balanceOf(address(_jose)) == 0, "Balance not 0");
        assertTrue(
            address(_jose).balance > avaxBalanceBefore,
            "AVAX not received"
        );
        console.log(
            "AVAX received: ",
    ...
    function test_Revert_SwapNoSplitFromAVAX_StratMember_TradingNotStarted()
        public
    {
        vm.startPrank(_jose);
        _setUp({_mintStrat: true});
    ...
    function test_Revert_SwapNoSplitFromAVAX_NonStratMember_TradingStarted()
        public
    {
        vm.startPrank(_jose);
        _setUp({_mintStrat: false});
    ...
    function test_Revert_AntiWhale_ExceedsMaximumHolding_TradingStarted_LessThan24Hours_NonStratMember()
        public
    {
        vm.startPrank(_user);
        _setUp({_mintStrat: false});
    ...
    function test_MultiSwaps_TradingStarted_MoreThan24Hours() public {
        ...
        assertTrue(
            token.balanceOf(address(_user)) == trade.amountOut,
            "AmountOut not received"
        );
        vm.stopPrank();
        ...
        assertTrue(
            token.balanceOf(address(_hitesh)) == trade.amountOut,
            "AmountOut not received"
        );
        ...
        assertTrue(token.balanceOf(address(_hitesh)) == 0, "Balance not 0");
        assertTrue(
            address(_hitesh).balance > avaxBalanceBefore,
            "AVAX not received"
        );
        console.log(
            "AVAX received: ",
        ...
        assertTrue(token.balanceOf(address(_user)) == 0, "Balance not 0");
        assertTrue(
            address(_user).balance > avaxBalanceBefore,
            "AVAX not received"
        );
        console.log(
            "AVAX received: ",
